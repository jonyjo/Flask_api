...
def extract_features_grd(image_path, models):
    image = load_img(image_path, target_size=(224, 224))  # Resize to fit model
...
def preprocess_and_extract(dataset_folder,img_folder, feat_folder, aug_count=1):
    ...
    with tqdm(total=num_images, desc="Preprocessing and Extracting Features") as progress_bar:
        ...
        for filename in os.listdir(dataset_folder):
            if filename.endswith(".jpg"):
                ...
                # Segment the image
                seg_img = segment_image_kmeans(image_path)
...
def load_data(dataset_folder, num_classes):
    X, y = [], []
    ...
    for filename in os.listdir(dataset_folder):
        if filename.endswith(".npy"):
            label = filename.split()[0]  # Assuming the label is part of the filename
            feature_path = os.path.join(dataset_folder, filename)
...
preprocess_and_extract(data_folder,img_folder, feat_folder)
...
model.fit([X_train_img, X_train_feat], y_train, batch_size=4, epochs=1,validation_split=0.2)
...
with tqdm(total=num_images, desc="Extracting Features for grade classification model") as progress_bar:
    ...
    for img_path in df['image_path']:
        features = extract_features_grd(img_path, models)
...
# Encode labels
label_encoder = LabelEncoder()
encoded_labels = label_encoder.fit_transform(df['grade'])
